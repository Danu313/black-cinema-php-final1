...
$queryPayment = mysqli_query($conn, "SELECT * FROM payment WHERE userId = '$userId' AND status = 'pending'");
...
if (!empty($promoCode)) {
    // Query untuk mendapatkan detail promo
    ...
    if (mysqli_num_rows($queryPromo) > 0) {
        ...
        // Tampilkan pesan promo (jika ada) dan harga setelah diskon
...
<?php
        ...
        // Tampilkan pesan jika kode promo tidak valid
        ...
        <div class="flex flex-col w-full justify-center items-center pt-[100px]">
            ...
            <?php if (mysqli_num_rows($resultPending) > 0) { ?>
                <div>
                    ...
                    <?php while ($resultPayment = mysqli_fetch_assoc($queryPayment)) { ?>
                        <?php if ($resultPayment['methodPayment'] === '' && $resultPayment['status'] === 'pending') { ?>
                            <div class='flex flex-col items-center justify-center'>
                                <div id="payment-method" class='flex flex-col w-[70vw] lg:w-[40vw] pb-2 items-center'>
                                    ...
                                    ?>
                                        <div class='flex flex-row w-full h-20 justify-between rounded-tr-lg rounded-tl-lg <?= $isChecked ? 'bg-gray-200 dark:bg-gray-800' : 'bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer' ?>' onClick='handleClick(<?= $index ?>)'>
                                            ...
                                            <input type="radio" class="card-select mr-3" name="cardSelection" value="<?= $rowCard['nameCard'] ?>" <?= $isChecked ?> data-imageqr="<?= $rowCard['imageQR'] ?>" required />
                                        </div>
                                ...
                                <div id="qr-code" class="hidden w-[70vw] lg:w-[40vw]">
                                    <p class="font-normal text-base leading-7 text-gray-500 text-center mb-5 mt-6">Scan QR code ini untuk pembayaran</p>
                                    <img id="qr-image" src="" alt="QR Code" class="w-[200px] h-[200px] object-contain mx-auto">
                                </div>
                                <div id="checkout-button" class="max-lg:max-w-lg max-lg:mx-auto">
                                    ...
                                    <button id="checkout" class="rounded-full py-4 px-6 bg-indigo-600 text-white font-semibold text-lg w-full text-center transition-all duration-500 hover:bg-indigo-700">Checkout</button>
                                </div>
                            </div>
                        <?php } else { ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const cardRadios = document.querySelectorAll('.card-select');
        ...
        checkoutButton.addEventListener('click', function() {
            ...
            cardRadios.forEach(radio => {
                if (radio.checked) {
                    selectedImageQR = radio.getAttribute('data-imageqr');
                }
            });

            // Sembunyikan bagian checkout, tampilkan QR code
            checkoutDiv.style.display = 'none';
            qrCodeDiv.style.display = 'block';

            // Setel gambar QR code yang dipilih
            qrImage.src = selectedImageQR;
        });
    });
</script>